...
const DOI = "slm:screen-time-and-sleep-its-different-for-adults";
...
async function main() {
  ...
  const out: string[] = [];
  out.push(`-- Idempotent prod seed: Jamie Zeitzer's Stanford LM interview on screens before bed`);
  out.push(`-- ("Screen Time and Sleep — It's Different for Adults") + his approved interpretation.`);
  out.push(`-- Corrects the record so the /sleep "screens before bed" question returns Jamie's`);
  out.push(`-- nuanced view and is legitimately faculty-verified. Safe to re-run.`);
  out.push(`-- Paste into the Replit Database pane (production) once.`);
  out.push(`-- DOI: ${DOI}`);
  out.push(``);
...
FROM sources s WHERE s.doi=${sqlStr(src.doi)};`);
  ...
  const path = "/home/runner/workspace/.local/zeitzer-screens-prod-seed.sql";
